...
export const register = (full_name, email, password) =>
  client.post('/api/auth/register', { full_name, email, password }).then((r) => r.data)
...
export const uploadScan = (file, location, depthM, onProgress, coords) => {
...
export const getMapPoints = () => client.get('/api/map/points').then((r) => r.data.points || [])
export const getMapSectors = (cellDeg = 2) => client.get('/api/map/sectors', { params: { cell_deg: cellDeg } }).then((r) => r.data.sectors || [])
...
export const getScanFeedback = (scanId) =>
  client.get(`/api/history/${scanId}/feedback`).then((r) => r.data)
export const saveScanFeedback = (scanId, corrections, note) =>
  client.put(`/api/history/${scanId}/feedback`, { corrections, note }).then((r) => r.data)
