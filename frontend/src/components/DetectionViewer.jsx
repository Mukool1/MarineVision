...
import { getScanFeedback, saveScanFeedback } from "../api/api";
...
export default function DetectionViewer({ result, onUpdated }) {
        ...
        <div className="grid flex-1 place-items-center text-center">
          <div>
            ...
            <p className="m-0 text-sm text-muted">
              Process a sonar image to review model detections.
            </p>
          </div>
        </div>
      </section>
  ...
  const change = (id, key, value) => {
    ...
    setCorrections((current) => {
      ...
      return [
        ...current.filter((row) => row.id !== String(id)),
        { ...item, [key]: value },
      ];
    });
  };
  ...
  const save = async () => {
    ...
    setFeedbackError("");
    try {
      const response = await saveScanFeedback(
        result.scan_id,
        corrections,
        note,
      );
      ...
      setFeedbackError(
        error?.response?.data?.detail ||
          "Feedback could not be saved. Restart the API once to enable the feedback endpoint.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section className="panel">
      ...
      {view === "annotated" ? (
        <div className="relative mt-4 overflow-hidden rounded-xl border border-line bg-[#061c2c]">
          <img
          ...
          {imageSize &&
            detections.map((detection) => {
              ...
              if (item.status === "false_positive" || item.excluded)
                return null;
        ...
        <img
          ...
          className="mt-4 max-h-[430px] w-full rounded-xl border border-line bg-[#f6faf9] object-contain"
        />
      )}
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Stat
          value={summary.severity_counts?.high || 0}
          label="High priority"
          alert
        />
        <Stat
      ...
      <div className="mt-5">
        ...
        {detections.length ? (
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span
                      className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase ${item.severity === "high" ? "bg-rose-100 text-rose-600" : item.severity === "medium" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                    >
                    ...
                    <div className="mt-3 grid gap-2 border-t border-line pt-3 sm:grid-cols-3">
                      <label>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            change(detection.id, "label", e.target.value)
                          }
                        >
                          {[
                            detection.label,
                            ...categories.filter((x) => x !== detection.label),
                          ].map((value) => (
                      ...
                      <label>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            change(detection.id, "severity", e.target.value)
                          }
                        >
                      ...
                      <label>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            change(detection.id, "status", e.target.value)
                          }
                        >
          ...
          <p className="text-sm text-muted">
            No targets met the current confidence threshold.
          </p>
        )}
      </div>
      {feedbackOpen && (
        <div className="mt-4 rounded-xl border border-line bg-cyan-400/5 p-4">
          ...
          <p className="mb-3 mt-1 text-[10px] leading-4 text-slate-500">
            Edit any category, priority, or decision above. The updated values
            apply immediately in this review and are stored with the scan.
          </p>
          <textarea
            value={note}
            onChange={(e) => {
          ...
          {feedbackError && (
            <p
              role="alert"
              className="mb-0 mt-3 rounded-lg bg-rose-50 p-2 text-[10px] text-rose-700"
            >
              {feedbackError}
            </p>
          )}
          <button
            onClick={save}
            disabled={saving}
            className="action mt-3 w-full gap-2 text-xs"
          >
